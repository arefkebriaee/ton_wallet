...
def convert_to_user_friendly(raw_address: str):
...
async def restore_v4r2_wallet_and_get_balance_seqno(mnemonic_list):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        url_transactions = f"{TONAPI_RPC}/blockchain/accounts/{user_friendly_address}/transactions?limit=10"
        try:
            async with session.get(url_transactions) as response:
                if response.status == 200:
                    ...
                    for index, tx in enumerate(transactions):
                        ...
                        if in_amount != 0:
                            ...
                            print("\n")
                            print(f"TX: {index + 1}:\n  hash: {tx_hash}\n   in amount: {in_amount} TON\n    out amount: {out_amount} TON\nsource: {convert_to_user_friendly(in_msg.get('source').get('address'))}\n  destination: {convert_to_user_friendly(in_msg.get('destination').get('address'))}\n  message: {message}")
